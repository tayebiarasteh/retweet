...
parameters = dict(lr = [.01, .001], batch_size = [5, 10])
...
for lr, BATCH_SIZE in product(*param_values):
    ...
    test_dataset = ConcatDataset([
        data_provider(dataset_name='2015_b_test_gold.txt', size=500, cfg_path=CFG_PATH, mode=Mode.PREDICT, seed=5),
        data_provider(dataset_name='2014_b_test_gold.txt', size=500, cfg_path=CFG_PATH, mode=Mode.PREDICT, seed=5)])
    ...
    trainer.setup_model(model='', optimiser=OPTIMIZER, optimiser_params=optimiser_params, loss_function=LOSS_FUNCTION)

    '''Execute Training'''
    trainer.execute_training(train_loader,test_loader,num_epochs=NUM_EPOCH)
